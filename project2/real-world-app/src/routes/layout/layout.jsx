import { NavLink } from "react-router-dom";

import { cs } from "../../common/chain-services";
import { consumeContext } from "../../common/react/context";
import { State } from "../../common/react/state";

export const Layout = ({ children }) => {
  return cs(
    consumeContext("auth"),
    ({auth}) => {
      const navLinks = getNavLinks(auth);
      return (
        <>
          <nav className="navbar navbar-light">
            <div className="container">
              <a className="navbar-brand" href="/">
                conduit
              </a>
              <ul className="nav navbar-nav pull-xs-left">
                <li className="nav-item">
                  <a className="nav-link" href="https://github.com/TonyMckes/conduit-realworld-example-app">
                    <i className="ion-social-github"></i> Source code
                  </a>
                </li>
              </ul>
              <ul className="nav navbar-nav pull-xs-right">
                {navLinks.map(({ to, label }) => (
                  <li className="nav-item" key={to}>
                    <NavLink className="nav-link" to={to}>
                      {label}
                    </NavLink>
                  </li>
                ))}

                {auth.user && ProfileDropdown({})}
              </ul>
            </div>
          </nav>

          {children}

          <footer>
            <div className="container">
              <a href="/" className="logo-font">
                conduit
              </a>
              <span className="attribution">
                An interactive learning project from{" "}
                <a href="https://thinkster.io">Thinkster</a>. Code &amp; design
                licensed under MIT.
              </span>
              <ul className="nav navbar-nav pull-xs-right">
                <li className="nav-item">
                  <a className="nav-link" href="https://github.com/TonyMckes/conduit-realworld-example-app">
                    <i className="ion-social-github"></i> Source code
                  </a>
                </li>
              </ul>
            </div>
          </footer>
        </>
      );
    }
  );
};

const ProfileDropdown = ({}) => {
  return cs(
    consumeContext("auth"),
    ["isClicked", ({}, next) => State({ initValue: false, next })],
    ({ auth, isClicked }) => {
      return (
        <li className="nav-item dropdown">
          <div 
            className="nav-link dropdown-toggle cursor-pointer" 
            style={{cursor: "pointer"}}
            onClick={() => {isClicked.onChange(!isClicked.value)}}
          >
            <img alt="pbl" className="user-pic" src="data:image/jpeg;base64,"/>
            {auth.user.username}
          </div>
          <div className="dropdown-menu" style={{display: isClicked.value ? "block" : "none"}}>
            <NavLink className="dropdown-item" to={`/profile/${auth.user.username}`}><i className="ion-person"></i> Profile</NavLink>
            <NavLink className="dropdown-item" to="/settings"><i className="ion-gear-a"></i> Settings</NavLink>
            <div className="dropdown-divider"></div>
            <button className="dropdown-item" onClick={() => {auth.logout()}}><i className="ion-log-out"></i> Logout</button>
          </div>
        </li>
      );
    }
  );
};

const getNavLinks = (auth) => {
  let links = [
    { to: "/", label: <><i className="ion-compose"/>&nbsp;Home</> },
  ];

  if (!auth.user) {
    const unauthNavLinks = [
      { to: "/login", label: <><i className="ion-log-in"></i>&nbsp;Login</> },
      { to: "/register", label: "Sign up" },
    ];
    links.push( ...unauthNavLinks);
  } else {
    const authNavLinks = [
      { to: "/editor", label: <><i className="ion-compose"></i>&nbsp;New Article</> },
    ]
    links.push( ...authNavLinks);
  }
  return links;
};

